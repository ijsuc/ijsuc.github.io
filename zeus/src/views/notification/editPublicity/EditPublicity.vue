  ...
  export default {
    ...
    methods: {
      ...
      fetchs( method, params = {}, id = 0 ){
        let url = '';
        if( method === 'create' ){
          url = '/publicities/publicity/create/';
        }else if( method === 'update' ){
          url = `/publicities/publicity/${id}/update/`;
        }else if( method === 'query' ){
          url = `/publicities/publicity/${id}/query/`;
        }
        ...
        if( method === 'query' ){
          ...
          .then( response => {
            ...
            if( !errcode ){
              let { publicity } = response.data;
              this.formValue = publicity;
              this.formValue.tags = publicity.tags.map( tag => tag.id );
              this.editor.ready( ()=>{
                this.editor.setContent( publicity.content );
              } )
              this.formValue.classify = publicity.classify.id.toString();
          ...
          .then(response => {
            ...
            if( !errcode ){
              if( method === 'update' ){
                this.$router.push({ name: 'NotificationEditPublicityReceivers', params: { id: id} });
              }

              if( method === 'create' ){
                this.$router.push({ name: 'NotificationEditPublicityReceivers', params: { id: response.data.id } });
              }
            }else{
              this.$Message.error( errmsg );
            }    
