...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      fetchWikis() {
        let self = this;
        util.notificationAxios.get( '/wikis/wiki/list/', {
          params: {
            page: this.page,
      ...
      fetchDelete( id = 0, index = -1 ){
        let self = this;
        const url = `/wikis/wiki/${id}/delete/`;
      ...
      routerLink( index ){
        if( ~index ){
          this.$router.push({name: 'NotificationEditWiki', params: { id: index }});
        }else{
          this.$router.push({name: 'NotificationCreateWiki'})
        }
      },

      detailPage( id ){
        this.$router.push({name: 'WikiDetailPage', params: { id: id }});
      },
