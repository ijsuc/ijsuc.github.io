...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      fetchNotifies() {
        let self = this;
        util.notificationAxios.get( '/notifies/notify/list/', {
          params: {
            page: this.page,
      ...
      fetchDelete( id = 0, index = -1 ){
        let self = this;
        const url = `/notifies/notify/${id}/delete/`;
      ...
      routerLink( index ){
        if( ~index ){
          this.$router.push({name: 'NotificationEditNotify', params: { id: index }});
        }else{
          this.$router.push({name: 'NotificationCreateNotify'})
        }
      },
