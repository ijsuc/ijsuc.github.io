...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      fetchReceivers(){
        util.notificationAxios.get(`/notifies/notify/${this.$route.params.id}/receiver/list/`, {
          responseType: 'json'
        }).then((response) => {
      ...
      fetchUpdateReceivers(){
        ...
        util.notificationAxios.post(`/notifies/notify/${id}/receiver/create/`, {
          ...
          let { errcode, errmsg } = response.data;
          if( !errcode ){
            this.$router.push({ name: 'NotificationEditNotifySend', params: { id: id } });
          }else{
            this.$Message.error( errmsg );
          }
        })
