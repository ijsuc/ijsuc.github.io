  ...
  export default {
    ...
    methods: {
      ...
      fetchs( method, params = {}, id = 0 ){
        let url = '';
        if( method === 'create' ){
          url = '/wikis/wiki/create/';
        }else if( method === 'update' ){
          url = `/wikis/wiki/${id}/update/`;
        }else if( method === 'query' ){
          url = `/wikis/wiki/${id}/query/`;
        }
        ...
        if( method === 'query' ){
          ...
          .then( response => {
            ...
            if( !errcode ){
              let { wiki } = response.data;
              this.formValue = wiki;
              this.formValue.tags = wiki.tags.map( tag => tag.id );
              this.editor.ready( ()=>{
                this.editor.setContent( wiki.content );
              } )
              this.formValue.classify = wiki.classify.id.toString();
          ...
          .then(response => {
            ...
            if( !errcode ){
              if( method === 'update' ){
                this.$router.push({ name: 'NotificationEditWikiReceivers', params: { id: id} });
              }

              if( method === 'create' ){
                this.$router.push({ name: 'NotificationEditWikiReceivers', params: { id: response.data.id } });
              }
            }else{
              this.$Message.error( errmsg );
            }    
