...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      fetchReceivers(){
        util.notificationAxios.get(`/wikis/wiki/${this.$route.params.id}/receiver/list/`, {
          responseType: 'json'
        }).then((response) => {
      ...
      fetchUpdateReceivers(){
        ...
        util.notificationAxios.post(`/wikis/wiki/${id}/receiver/create/`, {
          ...
          let { errcode, errmsg } = response.data;
          if( !errcode ){
            this.$router.push({ name: 'NotificationEditWikiSend', params: { id: id } });
          }else{
            this.$Message.error( errmsg );
          }
        })
